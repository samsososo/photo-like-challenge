  ...
  (photos, filters) => {
    ...
    if (filters.minWidth) {
      filtered = filtered.filter(
        (photo: Photo) => photo.width >= filters.minWidth!,
      );
    }

    if (filters.minHeight) {
      filtered = filtered.filter(
        (photo: Photo) => photo.height >= filters.minHeight!,
      );
    }
...
export const selectPhotosByDimension = createSelector(
  [selectPhotos],
  photos => {
    const small = photos.filter(
      (p: Photo) => p.width < 1000 && p.height < 1000,
    ).length;
    const medium = photos.filter(
      (p: Photo) =>
        (p.width >= 1000 && p.width < 2000) ||
        (p.height >= 1000 && p.height < 2000),
    ).length;
    const large = photos.filter(
      (p: Photo) => p.width >= 2000 || p.height >= 2000,
    ).length;
...
export const selectAuthorsList = createSelector([selectPhotos], photos => [
  ...new Set(photos.map((photo: Photo) => photo.author)),
]);

export const selectLegacyPhotos = (state: RootState) =>
  state.getAllPhotoReducers.photos;
export const selectLegacyCurrentPage = (state: RootState) =>
  state.getAllPhotoReducers.currentPage;
export const selectLegacyHasMore = (state: RootState) =>
  state.getAllPhotoReducers.hasMore;
export const selectLegacyIsLoading = (state: RootState) =>
  state.getAllPhotoReducers.isLoading;
